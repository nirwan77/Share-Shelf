...
function DashboardHome() {
    ...
    return (
      <Center h="60vh">
        <Stack align="center" gap="md">
          ...
          <Text c="dimmed" fw={500}>Loading your dashboard...</Text>
        </Stack>
      </Center>
    )
  }
    ...
    return (
      <Center h="60vh">
        <Paper p="xl" withBorder radius="md" style={{ textAlign: 'center' }}>
          <Text c="red" fw={700} size="lg">Error loading statistics</Text>
          <Text c="dimmed">Please make sure the backend is running and try again.</Text>
        </Paper>
      </Center>
    )
  }
  ...
  const stats = [
    { title: 'Total Users', value: data.overview.totalUsers, icon: IconUsers, color: 'blue' },
    { title: 'Total Books', value: data.overview.totalBooks, icon: IconBook, color: 'green' },
    { title: 'Total Genres', value: data.overview.totalGenres, icon: IconTags, color: 'grape' },
    { title: 'Community Posts', value: data.overview.totalPosts, icon: IconMessageCircle, color: 'orange' },
    { title: 'Total Revenue', value: `Rs. ${data.overview.totalRevenue.toLocaleString()}`, icon: IconCoin, color: 'teal' },
  ]
      ...
      <SimpleGrid cols={{ base: 1, lg: 2 }} spacing="xl">
        ...
        <Paper withBorder p="lg" radius="md" shadow="sm">
          <Group mb="lg" justify="space-between">
            ...
            <Badge variant="light" color="green">Newly Added</Badge>
          </Group>
          <Table verticalSpacing="sm">
            <Table.Thead>
              <Table.Tr>
            ...
            <Table.Tbody>
              {data.recent.books.map((book) => (
                <Table.Tr key={book.id}>
                  <Table.Td>
                    <Group gap="sm">
                      <Avatar src={book.image} size={26} radius="sm" variant="outline" />
                      <Text size="sm" fw={500}>{book.name}</Text>
                    </Group>
                  </Table.Td>
                  <Table.Td>
                  ...
                  <Table.Td>
                    <Text size="sm" fw={600}>Rs. {book.price}</Text>
                  </Table.Td>
                </Table.Tr>
              ))}
            </Table.Tbody>
        ...
        <Paper withBorder p="lg" radius="md" shadow="sm">
          <Group mb="lg" justify="space-between">
            ...
            <Badge variant="light" color="teal">Success</Badge>
          </Group>
          <Table verticalSpacing="sm">
            <Table.Thead>
              <Table.Tr>
            ...
            <Table.Tbody>
              {data.recent.payments.map((payment) => (
                <Table.Tr key={payment.id}>
                  <Table.Td>
                    <Text size="sm" fw={500}>{payment.user?.name || 'Unknown'}</Text>
                  </Table.Td>
                  <Table.Td>
                    <Text size="sm">Rs. {payment.total_amount}</Text>
                  </Table.Td>
                  <Table.Td>
                    <Badge color={payment.status === 'SUCCESS' ? 'green' : 'red'} size="sm" variant="outline">
                      {payment.status}
                    </Badge>
                  </Table.Td>
                  <Table.Td>
                    <Text size="sm" c="dimmed">{new Date(payment.createdAt).toLocaleDateString()}</Text>
                  </Table.Td>
                </Table.Tr>
              ))}
            </Table.Tbody>
